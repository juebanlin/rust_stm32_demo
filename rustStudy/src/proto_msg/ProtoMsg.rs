// This file is generated by rust-protobuf 2.23.0. Do not edit
// @generated

// https://github.com/rust-lang/rust-clippy/issues/702
#![allow(unknown_lints)]
#![allow(clippy::all)]

#![allow(unused_attributes)]
#![cfg_attr(rustfmt, rustfmt::skip)]

#![allow(box_pointers)]
#![allow(dead_code)]
#![allow(missing_docs)]
#![allow(non_camel_case_types)]
#![allow(non_snake_case)]
#![allow(non_upper_case_globals)]
#![allow(trivial_casts)]
#![allow(unused_imports)]
#![allow(unused_results)]
//! Generated file from `ProtoMsg.proto`

/// Generated files are compatible only with the same version
/// of protobuf runtime.
// const _PROTOBUF_VERSION_CHECK: () = ::protobuf::VERSION_2_23_0;

#[derive(PartialEq,Clone,Default)]
pub struct HelloWordRequest {
    // message fields
    pub hot_word_text: ::std::string::String,
    pub session_id: ::std::string::String,
    // special fields
    pub unknown_fields: ::protobuf::UnknownFields,
    pub cached_size: ::protobuf::CachedSize,
}

impl<'a> ::std::default::Default for &'a HelloWordRequest {
    fn default() -> &'a HelloWordRequest {
        <HelloWordRequest as ::protobuf::Message>::default_instance()
    }
}

impl HelloWordRequest {
    pub fn new() -> HelloWordRequest {
        ::std::default::Default::default()
    }

    // string hot_word_text = 1;


    pub fn get_hot_word_text(&self) -> &str {
        &self.hot_word_text
    }
    pub fn clear_hot_word_text(&mut self) {
        self.hot_word_text.clear();
    }

    // Param is passed by value, moved
    pub fn set_hot_word_text(&mut self, v: ::std::string::String) {
        self.hot_word_text = v;
    }

    // Mutable pointer to the field.
    // If field is not initialized, it is initialized with default value first.
    pub fn mut_hot_word_text(&mut self) -> &mut ::std::string::String {
        &mut self.hot_word_text
    }

    // Take field
    pub fn take_hot_word_text(&mut self) -> ::std::string::String {
        ::std::mem::replace(&mut self.hot_word_text, ::std::string::String::new())
    }

    // string session_id = 2;


    pub fn get_session_id(&self) -> &str {
        &self.session_id
    }
    pub fn clear_session_id(&mut self) {
        self.session_id.clear();
    }

    // Param is passed by value, moved
    pub fn set_session_id(&mut self, v: ::std::string::String) {
        self.session_id = v;
    }

    // Mutable pointer to the field.
    // If field is not initialized, it is initialized with default value first.
    pub fn mut_session_id(&mut self) -> &mut ::std::string::String {
        &mut self.session_id
    }

    // Take field
    pub fn take_session_id(&mut self) -> ::std::string::String {
        ::std::mem::replace(&mut self.session_id, ::std::string::String::new())
    }
}

impl ::protobuf::Message for HelloWordRequest {
    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::ProtobufResult<()> {
        while !is.eof()? {
            let (field_number, wire_type) = is.read_tag_unpack()?;
            match field_number {
                1 => {
                    ::protobuf::rt::read_singular_proto3_string_into(wire_type, is, &mut self.hot_word_text)?;
                },
                2 => {
                    ::protobuf::rt::read_singular_proto3_string_into(wire_type, is, &mut self.session_id)?;
                },
                _ => {
                    ::protobuf::rt::read_unknown_or_skip_group(field_number, wire_type, is, self.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u32 {
        let mut my_size = 0;
        if !self.hot_word_text.is_empty() {
            my_size += ::protobuf::rt::string_size(1, &self.hot_word_text);
        }
        if !self.session_id.is_empty() {
            my_size += ::protobuf::rt::string_size(2, &self.session_id);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.get_unknown_fields());
        self.cached_size.set(my_size);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::ProtobufResult<()> {
        if !self.hot_word_text.is_empty() {
            os.write_string(1, &self.hot_word_text)?;
        }
        if !self.session_id.is_empty() {
            os.write_string(2, &self.session_id)?;
        }
        os.write_unknown_fields(self.get_unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn get_cached_size(&self) -> u32 {
        self.cached_size.get()
    }

    fn get_unknown_fields(&self) -> &::protobuf::UnknownFields {
        &self.unknown_fields
    }

    fn mut_unknown_fields(&mut self) -> &mut ::protobuf::UnknownFields {
        &mut self.unknown_fields
    }

    fn as_any(&self) -> &dyn (::std::any::Any) {
        self as &dyn (::std::any::Any)
    }
    fn as_any_mut(&mut self) -> &mut dyn (::std::any::Any) {
        self as &mut dyn (::std::any::Any)
    }
    fn into_any(self: ::std::boxed::Box<Self>) -> ::std::boxed::Box<dyn (::std::any::Any)> {
        self
    }

    fn descriptor(&self) -> &'static ::protobuf::reflect::MessageDescriptor {
        Self::descriptor_static()
    }

    fn new() -> HelloWordRequest {
        HelloWordRequest::new()
    }

    fn descriptor_static() -> &'static ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::LazyV2<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::LazyV2::INIT;
        descriptor.get(|| {
            let mut fields = ::std::vec::Vec::new();
            fields.push(::protobuf::reflect::accessor::make_simple_field_accessor::<_, ::protobuf::types::ProtobufTypeString>(
                "hot_word_text",
                |m: &HelloWordRequest| { &m.hot_word_text },
                |m: &mut HelloWordRequest| { &mut m.hot_word_text },
            ));
            fields.push(::protobuf::reflect::accessor::make_simple_field_accessor::<_, ::protobuf::types::ProtobufTypeString>(
                "session_id",
                |m: &HelloWordRequest| { &m.session_id },
                |m: &mut HelloWordRequest| { &mut m.session_id },
            ));
            ::protobuf::reflect::MessageDescriptor::new_pb_name::<HelloWordRequest>(
                "HelloWordRequest",
                fields,
                file_descriptor_proto()
            )
        })
    }

    fn default_instance() -> &'static HelloWordRequest {
        static instance: ::protobuf::rt::LazyV2<HelloWordRequest> = ::protobuf::rt::LazyV2::INIT;
        instance.get(HelloWordRequest::new)
    }
}

impl ::protobuf::Clear for HelloWordRequest {
    fn clear(&mut self) {
        self.hot_word_text.clear();
        self.session_id.clear();
        self.unknown_fields.clear();
    }
}

impl ::std::fmt::Debug for HelloWordRequest {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for HelloWordRequest {
    fn as_ref(&self) -> ::protobuf::reflect::ReflectValueRef {
        ::protobuf::reflect::ReflectValueRef::Message(self)
    }
}

#[derive(PartialEq,Clone,Default)]
pub struct HelloWordResponse {
    // message fields
    pub errcode: i64,
    pub errmsg: ::std::string::String,
    pub session_id: ::std::string::String,
    pub hot_word_model: ::std::vec::Vec<u8>,
    // special fields
    pub unknown_fields: ::protobuf::UnknownFields,
    pub cached_size: ::protobuf::CachedSize,
}

impl<'a> ::std::default::Default for &'a HelloWordResponse {
    fn default() -> &'a HelloWordResponse {
        <HelloWordResponse as ::protobuf::Message>::default_instance()
    }
}

impl HelloWordResponse {
    pub fn new() -> HelloWordResponse {
        ::std::default::Default::default()
    }

    // int64 errcode = 1;


    pub fn get_errcode(&self) -> i64 {
        self.errcode
    }
    pub fn clear_errcode(&mut self) {
        self.errcode = 0;
    }

    // Param is passed by value, moved
    pub fn set_errcode(&mut self, v: i64) {
        self.errcode = v;
    }

    // string errmsg = 2;


    pub fn get_errmsg(&self) -> &str {
        &self.errmsg
    }
    pub fn clear_errmsg(&mut self) {
        self.errmsg.clear();
    }

    // Param is passed by value, moved
    pub fn set_errmsg(&mut self, v: ::std::string::String) {
        self.errmsg = v;
    }

    // Mutable pointer to the field.
    // If field is not initialized, it is initialized with default value first.
    pub fn mut_errmsg(&mut self) -> &mut ::std::string::String {
        &mut self.errmsg
    }

    // Take field
    pub fn take_errmsg(&mut self) -> ::std::string::String {
        ::std::mem::replace(&mut self.errmsg, ::std::string::String::new())
    }

    // string session_id = 3;


    pub fn get_session_id(&self) -> &str {
        &self.session_id
    }
    pub fn clear_session_id(&mut self) {
        self.session_id.clear();
    }

    // Param is passed by value, moved
    pub fn set_session_id(&mut self, v: ::std::string::String) {
        self.session_id = v;
    }

    // Mutable pointer to the field.
    // If field is not initialized, it is initialized with default value first.
    pub fn mut_session_id(&mut self) -> &mut ::std::string::String {
        &mut self.session_id
    }

    // Take field
    pub fn take_session_id(&mut self) -> ::std::string::String {
        ::std::mem::replace(&mut self.session_id, ::std::string::String::new())
    }

    // bytes hot_word_model = 4;


    pub fn get_hot_word_model(&self) -> &[u8] {
        &self.hot_word_model
    }
    pub fn clear_hot_word_model(&mut self) {
        self.hot_word_model.clear();
    }

    // Param is passed by value, moved
    pub fn set_hot_word_model(&mut self, v: ::std::vec::Vec<u8>) {
        self.hot_word_model = v;
    }

    // Mutable pointer to the field.
    // If field is not initialized, it is initialized with default value first.
    pub fn mut_hot_word_model(&mut self) -> &mut ::std::vec::Vec<u8> {
        &mut self.hot_word_model
    }

    // Take field
    pub fn take_hot_word_model(&mut self) -> ::std::vec::Vec<u8> {
        ::std::mem::replace(&mut self.hot_word_model, ::std::vec::Vec::new())
    }
}

impl ::protobuf::Message for HelloWordResponse {
    fn is_initialized(&self) -> bool {
        true
    }

    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::ProtobufResult<()> {
        while !is.eof()? {
            let (field_number, wire_type) = is.read_tag_unpack()?;
            match field_number {
                1 => {
                    if wire_type != ::protobuf::wire_format::WireTypeVarint {
                        return ::std::result::Result::Err(::protobuf::rt::unexpected_wire_type(wire_type));
                    }
                    let tmp = is.read_int64()?;
                    self.errcode = tmp;
                },
                2 => {
                    ::protobuf::rt::read_singular_proto3_string_into(wire_type, is, &mut self.errmsg)?;
                },
                3 => {
                    ::protobuf::rt::read_singular_proto3_string_into(wire_type, is, &mut self.session_id)?;
                },
                4 => {
                    ::protobuf::rt::read_singular_proto3_bytes_into(wire_type, is, &mut self.hot_word_model)?;
                },
                _ => {
                    ::protobuf::rt::read_unknown_or_skip_group(field_number, wire_type, is, self.mut_unknown_fields())?;
                },
            };
        }
        ::std::result::Result::Ok(())
    }

    // Compute sizes of nested messages
    #[allow(unused_variables)]
    fn compute_size(&self) -> u32 {
        let mut my_size = 0;
        if self.errcode != 0 {
            my_size += ::protobuf::rt::value_size(1, self.errcode, ::protobuf::wire_format::WireTypeVarint);
        }
        if !self.errmsg.is_empty() {
            my_size += ::protobuf::rt::string_size(2, &self.errmsg);
        }
        if !self.session_id.is_empty() {
            my_size += ::protobuf::rt::string_size(3, &self.session_id);
        }
        if !self.hot_word_model.is_empty() {
            my_size += ::protobuf::rt::bytes_size(4, &self.hot_word_model);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.get_unknown_fields());
        self.cached_size.set(my_size);
        my_size
    }

    fn write_to_with_cached_sizes(&self, os: &mut ::protobuf::CodedOutputStream<'_>) -> ::protobuf::ProtobufResult<()> {
        if self.errcode != 0 {
            os.write_int64(1, self.errcode)?;
        }
        if !self.errmsg.is_empty() {
            os.write_string(2, &self.errmsg)?;
        }
        if !self.session_id.is_empty() {
            os.write_string(3, &self.session_id)?;
        }
        if !self.hot_word_model.is_empty() {
            os.write_bytes(4, &self.hot_word_model)?;
        }
        os.write_unknown_fields(self.get_unknown_fields())?;
        ::std::result::Result::Ok(())
    }

    fn get_cached_size(&self) -> u32 {
        self.cached_size.get()
    }

    fn get_unknown_fields(&self) -> &::protobuf::UnknownFields {
        &self.unknown_fields
    }

    fn mut_unknown_fields(&mut self) -> &mut ::protobuf::UnknownFields {
        &mut self.unknown_fields
    }

    fn as_any(&self) -> &dyn (::std::any::Any) {
        self as &dyn (::std::any::Any)
    }
    fn as_any_mut(&mut self) -> &mut dyn (::std::any::Any) {
        self as &mut dyn (::std::any::Any)
    }
    fn into_any(self: ::std::boxed::Box<Self>) -> ::std::boxed::Box<dyn (::std::any::Any)> {
        self
    }

    fn descriptor(&self) -> &'static ::protobuf::reflect::MessageDescriptor {
        Self::descriptor_static()
    }

    fn new() -> HelloWordResponse {
        HelloWordResponse::new()
    }

    fn descriptor_static() -> &'static ::protobuf::reflect::MessageDescriptor {
        static descriptor: ::protobuf::rt::LazyV2<::protobuf::reflect::MessageDescriptor> = ::protobuf::rt::LazyV2::INIT;
        descriptor.get(|| {
            let mut fields = ::std::vec::Vec::new();
            fields.push(::protobuf::reflect::accessor::make_simple_field_accessor::<_, ::protobuf::types::ProtobufTypeInt64>(
                "errcode",
                |m: &HelloWordResponse| { &m.errcode },
                |m: &mut HelloWordResponse| { &mut m.errcode },
            ));
            fields.push(::protobuf::reflect::accessor::make_simple_field_accessor::<_, ::protobuf::types::ProtobufTypeString>(
                "errmsg",
                |m: &HelloWordResponse| { &m.errmsg },
                |m: &mut HelloWordResponse| { &mut m.errmsg },
            ));
            fields.push(::protobuf::reflect::accessor::make_simple_field_accessor::<_, ::protobuf::types::ProtobufTypeString>(
                "session_id",
                |m: &HelloWordResponse| { &m.session_id },
                |m: &mut HelloWordResponse| { &mut m.session_id },
            ));
            fields.push(::protobuf::reflect::accessor::make_simple_field_accessor::<_, ::protobuf::types::ProtobufTypeBytes>(
                "hot_word_model",
                |m: &HelloWordResponse| { &m.hot_word_model },
                |m: &mut HelloWordResponse| { &mut m.hot_word_model },
            ));
            ::protobuf::reflect::MessageDescriptor::new_pb_name::<HelloWordResponse>(
                "HelloWordResponse",
                fields,
                file_descriptor_proto()
            )
        })
    }

    fn default_instance() -> &'static HelloWordResponse {
        static instance: ::protobuf::rt::LazyV2<HelloWordResponse> = ::protobuf::rt::LazyV2::INIT;
        instance.get(HelloWordResponse::new)
    }
}

impl ::protobuf::Clear for HelloWordResponse {
    fn clear(&mut self) {
        self.errcode = 0;
        self.errmsg.clear();
        self.session_id.clear();
        self.hot_word_model.clear();
        self.unknown_fields.clear();
    }
}

impl ::std::fmt::Debug for HelloWordResponse {
    fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
        ::protobuf::text_format::fmt(self, f)
    }
}

impl ::protobuf::reflect::ProtobufValue for HelloWordResponse {
    fn as_ref(&self) -> ::protobuf::reflect::ReflectValueRef {
        ::protobuf::reflect::ReflectValueRef::Message(self)
    }
}

static file_descriptor_proto_data: &'static [u8] = b"\
    \n\x0eProtoMsg.proto\"[\n\x10HelloWordRequest\x12$\n\rhot_word_text\x18\
    \x01\x20\x01(\tR\x0bhotWordTextB\0\x12\x1f\n\nsession_id\x18\x02\x20\x01\
    (\tR\tsessionIdB\0:\0\"\x94\x01\n\x11HelloWordResponse\x12\x1a\n\x07errc\
    ode\x18\x01\x20\x01(\x03R\x07errcodeB\0\x12\x18\n\x06errmsg\x18\x02\x20\
    \x01(\tR\x06errmsgB\0\x12\x1f\n\nsession_id\x18\x03\x20\x01(\tR\tsession\
    IdB\0\x12&\n\x0ehot_word_model\x18\x04\x20\x01(\x0cR\x0chotWordModelB\0:\
    \0B\0b\x06proto3\
";

static file_descriptor_proto_lazy: ::protobuf::rt::LazyV2<::protobuf::descriptor::FileDescriptorProto> = ::protobuf::rt::LazyV2::INIT;

fn parse_descriptor_proto() -> ::protobuf::descriptor::FileDescriptorProto {
    ::protobuf::Message::parse_from_bytes(file_descriptor_proto_data).unwrap()
}

pub fn file_descriptor_proto() -> &'static ::protobuf::descriptor::FileDescriptorProto {
    file_descriptor_proto_lazy.get(|| {
        parse_descriptor_proto()
    })
}
